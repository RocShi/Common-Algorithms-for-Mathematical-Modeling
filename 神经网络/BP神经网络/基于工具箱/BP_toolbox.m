...
p=[sqrs;sqjdcs;sqglmj];
 % 目标数据矩阵
t=[glkyl;glhyl];   
% 利用mapminmax函数对数据进行归一化，源程序用得是premnmx
[pn,input_str] = mapminmax(p) ;   %input_str：归一化特征，下同
[tn,output_str] = mapminmax(t) ;

% 建立BP神经网络，相对旧一点的MATLAB版本，新版本 newff 函数使用更简洁一些
% 但是本质和性能没有区别
net=newff(pn,tn,[3 7 2],{'purelin','logsig','purelin'});  %newff(输入矩阵，输出矩阵，[输入维度 隐含层节点数 输出维度]，{'输入层传递函数'，'隐含层传递函数'，'输出层传递函数'})
% 10轮回显示一次结果
...
net=train(net,pn,tn);  %利用原始输入数据与目标数据训练网络                   
% 利用训练好的网络，基于原始数据对BP网络仿真
an=sim(net,pn);  %网络训练完成后，基于原始输入数据进行仿真，达到仿真目标数据    

% 利用函数mapminmax把仿真得到的数据还原为原始的数量级
% 新版本推荐训练样本归一化和反归一化都使用 mapminmax 函数
a = mapminmax('reverse',an,output_str);  %将仿真得到的目标数据“逆归一化”至与原始目标数据相同维度
...
pnewn = mapminmax('apply',pnew,input_str);  %利用原始输入数据矩阵的归一化特征对测试数据进行归一化
% 利用归一化后的数据进行仿真
anewn=sim(net,pnewn);       
% 把仿真得到的数据还原为原始的数量级
